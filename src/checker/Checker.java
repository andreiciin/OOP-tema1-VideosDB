...
public final class Checker {
  ...
  public boolean compareQueries(
      final List<QueryTest> queryTests1, final List<QueryTest> queryTests2) {
   ...
   * @throws IOException in case of exceptions to reading / writing
   */
  public void iterateFiles(final String dirOutPath, final String dirRefPath, final String inputPath)
      throws IOException {
    int score = 0;
    int length;
      ...
      for (int i = 0; i < listOut.size(); i++) {
        ...
        if (compareQueries(listRefs.get(i), listOut.get(i))) {
          ...
          if (inputDir[i].getName().contains(Constants.LARGE)
            ...
            System.out.println(
                inputDir[i].getName() + s + "PASSED (+" + Constants.LARGE_TEST + ")");
            score += Constants.LARGE_TEST;
          } else {
            System.out.println(
                inputDir[i].getName() + s + "PASSED (+" + Constants.SINGLE_TEST + ")");
